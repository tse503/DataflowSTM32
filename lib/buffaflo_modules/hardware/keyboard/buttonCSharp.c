#include "buttonCSharp.h"

/* Read the keyboard button for C# (GPIO E, Pin 8)
 * Output 0: BUFFER type: audio output
 */

void initButtonCSharp(void) {
    ...
    // Set GPIO E Pin 8 Mode Register to 0b00 (Input)
    GPIOE->MODER = ((GPIOE->MODER & ~GPIO_MODER_MODER8_Msk) | (0x00 << GPIO_MODER_MODER8_Pos));

    // Set GPIO E Pin 8 Pull-Up/Push-Down Register to 0b01 (Pull-Up)
    GPIOE->PUPDR = ((GPIOE->PUPDR & ~GPIO_PUPDR_PUPD8_Msk) | (0x01 << GPIO_PUPDR_PUPD8_Pos));
}

// Return if button C is pressed
float isButtonCSharpPressed(void) {
...
void BFLO_processButtonCSharpModule(module_t * module) {
    // Read the state of the button
    float button = isButtonCSharpPressed();

    // Set the module's output as the state of the button
    *(float *)(module->outputs[0].data) = button;
}

uint32_t BFLO_initButtonCSharpModule(module_t * module, graph_t * graph, char * moduleName) {
    initButtonCSharp();
    ...
    *(float *)(module->outputs[0].data) = 0;
    ...
    module->process = BFLO_processButtonCSharpModule;

...
/* Read the keyboard button for E (GPIO E, Pin 11)
 * Output 0: BUFFER type: audio output
 */

void initButtonE(void) {
    ...
    GPIOE->MODER = ((GPIOE->MODER & ~GPIO_MODER_MODER11_Msk) | (0x00 << GPIO_MODER_MODER11_Pos));

    // Set GPIO E Pin 9 Pull-Up/Push-Down Register to 0b01 (Pull-Up)
    GPIOE->PUPDR = ((GPIOE->PUPDR & ~GPIO_PUPDR_PUPD11_Msk) | (0x01 << GPIO_PUPDR_PUPD11_Pos));
}

// Return if button E is pressed
float isButtonEPressed(void) {
    if (GPIOE->IDR & GPIO_IDR_ID11_Msk) {
        return 0.0f;
    } else {
        return 1.0f;
    }
...
void BFLO_processButtonEModule(module_t * module) {
    // Read the state of the button
    float button = isButtonEPressed();

    // Set the module's output as the state of the button
    *(float *)(module->outputs[0].data) = button;
}

uint32_t BFLO_initButtonEModule(module_t * module, graph_t * graph, char * moduleName) {
    initButtonDSharp();
    ...
    *(float *)(module->outputs[0].data) = 0;
    ...
    module->process = BFLO_processButtonEModule;
